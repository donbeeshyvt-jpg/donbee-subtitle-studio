...
API 金鑰請由你自己在前端設定並儲存：右上角「模型設定 → 文字模型供應者」→ 選對應服務、確認遠端使用同意 → 填入金鑰 → 按「儲存金鑰」→ 確認顯示「已存金鑰」→ 按「測試連線」。只填入欄位但沒儲存不算完成。測試通過後如需指定模型，再填名稱並按「儲存模型」。

Agent 會等待你完成儲存後才繼續；不要把金鑰貼進對話或包含金鑰的截圖。Agent 不代填、不要求你改用命令列／環境變數／直接編輯 secrets；驗證只看已設定狀態與連線結果，不讀出金鑰。
